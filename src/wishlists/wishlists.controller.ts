...
export class WishlistsController {
    ...
    @Post()
    create(@Body() createWishlistDto: CreateWishlistDto, @Req() { user }: IUserRequest,): Promise<WishList> {
    ...
    @Patch(':id')
    update(@Param('id') id: number, @Body() updateWishlistDto: UpdateWishlistDto, @Req() { user }: IUserRequest,): Promise<WishList> {
        return this.wishlistsService.update(id, updateWishlistDto, user.id);
    }

    @Delete(':id')
    deleteOne(@Param('id') id: number, @Req() { user }: IUserRequest,): Promise<WishList> {
